...
export function parseConfigValue(field: JSONSchema, value: SchemaValueType): SchemaValueType {
  if (field?.type === "boolean") {
...
export function applyDefaultValues(values: JsonObject, schema: JSONSchema): JsonObject {
  const finalValues = { ...values };
  
  if (schema.properties) {
    Object.entries(schema.properties).forEach(([key, field]) => {
      // If the field is empty/undefined and has a default value, use the default
      if ((finalValues[key] === undefined || finalValues[key] === "") && 
          typeof field === 'object' && field && 'default' in field) {
