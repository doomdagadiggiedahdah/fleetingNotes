...
export async function tracedOpenAIParse<
	Params extends ChatCompletionParseParams,
>(client: OpenAI, trace: LangfuseTraceClient, params: Params) {
	const generation = trace.generation({
	...
	const response = await client.beta.chat.completions.parse(params)
