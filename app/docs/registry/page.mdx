...
- `tools`: An array of tools that this server provides, or `null` if no tools have been retrieved by smithery
  - Note that these tools are cached from the server's last deployment or scan and may not match exactly what the MCP server will return at runtime. The actual tools returned by the MCP server may differ based on configuration or changes to the server.
