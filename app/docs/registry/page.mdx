...
The Smithery Registry provides a RESTful API for managing and obtaining deployed Model Context Protocol (MCP) servers. This documentation will guide you through using the Registry API to obtain a WebSocket MCP server.

Please note: Smithery is still in early stages of development and the API may change in the future.
...
### Authentication
All endpoints require authentication via a bearer token. Currently, you have to request for an API key by [contacting us](mailto:contact@smithery.ai). Include this token in your API requests:
...
```http
WS `${url}/ws?config=${base64encode(config)}`
```
