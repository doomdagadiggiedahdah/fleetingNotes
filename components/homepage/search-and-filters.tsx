...
export function SearchAndFilters({
	...
	// Extract base query and current filter
	const baseQuery = query.replace(/\s*(is|has):\w+/, "").trim()
	const currentFilter =
		FILTER_OPTIONS.find((option) => query.includes(option.value))?.value || ""
		...
		<div className="space-y-4">
						...
						<div className="p-2">
							{FILTER_OPTIONS.map((option) => (
								<button
									...
									key={option.value}
									className={`
                    flex w-full items-start justify-between p-2 rounded-md cursor-pointer
			...
			{currentFilter && (
				<div className="flex flex-wrap gap-2">
					<div className="inline-flex items-center gap-1.5 bg-secondary/80 text-secondary-foreground px-3 py-1.5 rounded-full text-sm">
						{(() => {
